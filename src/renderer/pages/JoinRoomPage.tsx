  ...
  const fetchActiveRooms = async () => {
    try {
      ...
      const response: ApiResponse<RoomType[]> =
        await apiService.get('/api/rooms/active');
  ...
  const handleJoinRoom = useCallback(
    async (code: string) => {
      ...
      try {
        setIsLoading(true);
        const apiService = getApiService();
        const response: ApiResponse<RoomType & { playerId: number }> =
          await apiService.post('/api/rooms/join', {
            roomCode: code.trim(),
        ...
        if (response.success && response.data) {
          ...
          localStorage.setItem(
            LocalStorageKeyEnum.PLAYER_ID,
            response.data.playerId.toString(),
          );
    ...
    >
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-black/60 backdrop-blur-sm rounded-lg p-8 border-2 border-orange-600 mb-6">
          <h2 className="text-2xl font-bold text-orange-50 mb-6">
            Enter Room Code
          </h2>

          <div className="space-y-4">
            <div>
              <label className="block text-orange-50 mb-2 text-lg">
                Your Username
              </label>
              <Input
                type="text"
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {activeRooms.length === 0 ? (
              ...
              activeRooms.map((room) => {
                const filledSlots = room.players.filter(
                  (p) => p.name !== null,
                ).length;
                ...
                return (
                  ...
                  >
                    <div className="flex-1">
                      <h3 className="text-orange-50 font-bold text-lg">
                        Room #{room.id}
                      </h3>
                      ...
                      <p className="text-orange-300 text-sm">
                        Players: {filledSlots}/{totalSlots} | Timer:{' '}
                        {room.timerLimit}s
                      </p>
                    </div>
                    <Button
